...
# Ajouter la function chemin entr deux neoud 
...
def CheminEntreDeuxNoeuds(racine, info_a, info_b):
    ...
    chemin_final = []
    for x in chemin_a:
        chemin_final.append(x.info)
        if x == ancetre:
            break

    chemin_b = chemin_b[:chemin_b.index(ancetre)]
    chemin_final.extend(reversed([x.info for x in chemin_b]))
